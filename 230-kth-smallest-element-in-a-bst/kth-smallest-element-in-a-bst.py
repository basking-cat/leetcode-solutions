...
class Solution:
    ...
    def kthSmallestRecursive(self, root: Optional[TreeNode], k: int) -> int:
        # return node.val in each step
        # repeat k times, and then return the final answer

        inorder = []
        def dfs(node: Optional[TreeNode]): # define dfs function since we have to keep stack and inorder globally
            if not node:
                return
            dfs(node.left)
            inorder.append(node) # we don't have to define a stack since there'll be one automatically
            dfs(node.right)
